...
func NewRootCmd() *cobra.Command {
	...
	simApp := app.NewSimApp(log.NewNopLogger(), dbm.NewMemDB(), nil, true, simtestutil.NewAppOptionsWithFlagHome(app.DefaultNodeHome))
...
func appExport(
	...
	if height != -1 {
		testApp = app.NewSimApp(logger, db, traceStore, false, appOpts)

		if err := testApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		testApp = app.NewSimApp(logger, db, traceStore, true, appOpts)
	}
